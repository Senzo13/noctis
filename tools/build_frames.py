...
Usage : python tools/build_frames.py [filtre]

`filtre` est un fragment de nom de dossier (ex. « intro ») : seules les
séquences correspondantes sont (re)générées. Sans argument, tout est refait.
"""
...
SEQUENCES = [
    # Vidéo 0 — l'intro. C'est un plan à part (son propre fichier source), avec
    # sa propre caméra : elle se raccorde au début du rush, mesuré image par
    # image (voir HERO_START dans assets/js/main.js).
    ("assets/video/intro.mp4", "assets/frames/intro", 1600, 80, 2, None),
    ("assets/video/intro.mp4", "assets/frames/intro-mobile", 720, 80, 2, "crop=ih*9/16:ih"),
...
def main() -> int:
    ...
    seulement = sys.argv[1] if len(sys.argv) > 1 else ""

    for source, cible, largeur, qualite, pas, extra in SEQUENCES:
        if seulement and seulement not in cible:
            continue
        entree = ROOT / source
        sortie = ROOT / cible
